...
NS_LOG_COMPONENT_DEFINE ("sc2");

double sensor=1;
double anchor=3;
double overall_energy = 200;
...
ofstream ofs1 ("Scenario_2.plt", ofstream::out);
ofstream ofs2 ("Scenario_2.txt",ofstream::out);
...
/* void RemainingEnergy (Ptr<BasicEnergySource> basicSourcePtr,Ptr<BasicEnergySource> basicSourcePtr1,Ptr<BasicEnergySource> basicSourcePtr2,Ptr<BasicEnergySource> basicSourcePtr3)
{
  NS_LOG_UNCOND("Time "<<Simulator::Now ().GetSeconds ()<<" s "<< " Remaining energy of A0: " << basicSourcePtr1 -> GetRemainingEnergy() <<" A1: "<<basicSourcePtr2 -> GetRemainingEnergy()<<" A2: "<<basicSourcePtr3 -> GetRemainingEnergy()<<" Sensor: "<<basicSourcePtr -> GetRemainingEnergy() );
  ofs2 << Simulator::Now ().GetSeconds ()<< " " << basicSourcePtr1 -> GetRemainingEnergy()<< " " <<basicSourcePtr2 -> GetRemainingEnergy()<< " " <<basicSourcePtr3 -> GetRemainingEnergy()<< " " <<basicSourcePtr -> GetRemainingEnergy()<<endl;
  //Simulator::Schedule (Seconds (0.1), &RemainingEnergy, basicSourcePtr,basicSourcePtr1,basicSourcePtr2,basicSourcePtr3);
...
int main (int argc, char *argv[])
{
    ...
    ofs1 << "set terminal png"<<endl;
	  ofs1 << "set output 'Scenario_2.png'"<<endl;
	  ofs1 << "set title 'Scenario_2'"<<endl;
	  ofs1 << "set xlabel 'Time (Second)' "<<endl;
	  ofs1 << "set ylabel 'Energy (J)' "<<endl;	 
    ofs1 << "set xrange [0:100]"<<endl;
    ofs1 << "set yrange [0:60]"<<endl;
	  ofs1 << "plot "<<" 'Scenario_2.txt' " <<"using 1:2 with linespoints title'Anchor0',"<<"'Scenario_2.txt' using 1:3 with linespoints title 'Anchor1',"<<"'Scenario_2.txt' using 1:4 with linespoints title 'Anchor2',"<<"'Scenario_2.txt' using 1:5 with linespoints title 'Sensor0'"<<endl;
   ...
   NodeContainer SensorNodes;
    ...
    Ptr<UniformRandomVariable> randomizer1 = CreateObject<UniformRandomVariable> ();
	  ...
	  randomizer1->SetAttribute ("Max", DoubleValue (50));
  ...
  //basicSourceHelper.Set("BasicEnergySupplyVoltageV",DoubleValue (5.0));//setting initial voltage
	// install source
  ...
  DeviceEnergyModelContainer deviceModels1 = radioEnergyHelper.Install (anchorDevice, sources1);
   ...
   Ptr<WifiRadioEnergyModel> model1 =m_deviceEnergyModel.Create<WifiRadioEnergyModel> (); 
    ...
    if (udp)
      ...
      pAnim = new AnimationInterface ("sc2.xml");   //NetAnim file creation
